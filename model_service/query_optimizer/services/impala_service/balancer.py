...
class AdvancedImpalaBalancer:
    ...
    def balance(self, ml_output, cluster_state):
        ...
        pred_dop = ml_output['mt_dop']
        mem_dop0 = self._parse_mem(ml_output['base_pmu'])
        mem_mt_dop = self._parse_mem(ml_output['pred_mem_limit'])
        ...
        mem_p = (cluster_state.get('admitted_mem_gb', 0) /
                 (cluster_state.get('max_mem_gb', 1) * 100 if cluster_state.get('max_mem_gb', 1) != 0 else 1))

        # 3. ГИБКАЯ КОРРЕКЦИЯ DOP
        current_dop = pred_dop

        # Оцениваем "тесноту" в кластере: сколько запросов на одну живую ноду
        queries_per_node = running_q / (live_nodes +1)
        ...
        if pred_dop > 1 and current_dop < pred_dop:
            ...
            ratio = (current_dop - 1) / (pred_dop - 1) if pred_dop != 1 else 1
        ...
        final_mem_per_node = (interpolated_mem_3nodes * self.train_nodes) / live_nodes if live_nodes != 0 else 1
        ...
        if cluster_state.get('queued_queries', 0) > 10 or mem_p > 90:
            ...
            final_mem_per_node = (mem_dop0 * self.train_nodes) / live_nodes if live_nodes != 0 else 1
        ...
        return {
            'mem_limit': f"{max(256, final_mem_mb)}mb",
            'mt_dop': int(current_dop),
            'num_scanner_threads': ml_output['threads'] if current_dop > 1 else 1,
            'default_join_destribution_mode': ml_output['join_mode'],
            'default_disable_codegen': ml_output['codegen'],
            'balancer_log': {
