...
class ImpalaCascadeEnsemble:
    ...
    def train(self, df_train_raw):
        ...
        for leaf_id in df_train['leaf_id'].unique():
            ...
            if not opt.empty:
                ...
                base_pmu = pd.to_numeric(opt['max_mem_limit_dop0'], errors='coerce').fillna(1)

                pmu_ratio = (actual_pmu / base_pmu).replace([np.inf, -np.inf], 1.0).fillna(1.0)

                self.leaf_profiles[leaf_id] = {
                    'pmu_adj': float(np.percentile(pmu_ratio, 90)),  # Квантиль 90
    ...
    def predict(self, plan_dict):
        ...
        if not profile or profile['success_rate'] < 0.4:  # Если профиль не найден или ненадежен
