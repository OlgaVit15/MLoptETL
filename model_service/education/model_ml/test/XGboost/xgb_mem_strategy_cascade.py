...
from .xgb_mem_regressor import MemoryLimitPredictor
from .xgb_strategy_classifier import StrategyCascadeClassifier

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)


class XGBImpalaOptimizationCascade:
    """
    Каскадная модель:
    ...
    def predict_s(self, row: dict) -> dict:
        ...
        pred_base_pmu = self.regressor.predict_s(row)[0]
    ...
    def save(self, folder_path: str = "models"):
        ...
        file_path = os.path.join(folder_path, f"xgbimpala_cascade_{timestamp}.joblib")
    ...
    def load(file_path: str):
        ...
        if not isinstance(model, XGBImpalaOptimizationCascade):
