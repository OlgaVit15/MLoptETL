...
from model_ml.test.LGBM.lgbm_mem_regressor import MemoryLimitPredictor
from model_ml.test.LGBM.lgbm_strategy_classifier import StrategyCascadeClassifier

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)


class LGBMImpalaOptimizationCascade:
    """
    Каскадная модель:
    ...
    def load_and_clean_data(self, uri: str, table_name: str) -> pd.DataFrame:
        ...
        logger.info(f"Загрузка данных из {table_name}...")
    ...
    def evaluate_cascade(self, df_test: pd.DataFrame):
        ...
        acc_dop = accuracy_score(df_test['target_mt_dop'], results['mt_dop'])        # Точность джойнов
        acc_join = accuracy_score(df_test['target_default_join_distribution_mode'], results['join_mode'])
        # Точность threads
        acc_t = accuracy_score(df_test['target_num_scanner_threads'], results['threads'])
        # Точность codegen
        acc_cg = accuracy_score(df_test['target_disable_codegen'], results['codegen'])
        ...
        print(f"Accuracy (JOIN Selection):  {acc_join:.2%}")
        print(f"Accuracy (THREADS Selection):  {acc_t:.2%}")
        print(f"Accuracy (CODEGEN Selection):  {acc_cg:.2%}")
    ...
    def save(self, folder_path: str = "models"):
        ...
        file_path = os.path.join(folder_path, f"lgbimpala_cascade_{timestamp}.joblib")
    ...
    def load(file_path: str):
        ...
        if not isinstance(model, LGBMImpalaOptimizationCascade):
            raise ValueError("Файл не является экземпляром LGBMImpalaOptimizationCascade")
