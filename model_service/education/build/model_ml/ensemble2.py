...
class ImpalaCascadeEnsemble:
    ...
    def train(self, df_train_raw):
        ...
        self.model_strategy = RandomForestRegressor(
            n_estimators=self.n_estimators,
            min_samples_leaf=calculated_min_leaf,
            random_state=42, n_jobs=-1
        )
        ...
        # 2. Формируем "Бронированные" профили
        for leaf_id in df_train['leaf_id'].unique():
            ...
            if not opt.empty:
                ...
                # pmu_ratio = (opt['target_mem_limit'] / opt['max_mem_limit_dop0']).replace([np.inf, -np.inf], 1.0).fillna(1.0)

                # Более точный вариант для обучения "Брони"
                actual_pmu = pd.to_numeric(opt['metric_pmu'], errors='coerce').fillna(0)
                base_pmu = pd.to_numeric(opt['max_mem_limit_dop0'], errors='coerce').fillna(1)  # замена 0 на 1

                pmu_ratio = (actual_pmu / base_pmu).replace([np.inf, -np.inf], 1.0).fillna(1.0)

                self.leaf_profiles[leaf_id] = {
                    'pmu_adj': float(np.percentile(pmu_ratio, 90)),  # Квантиль 90 вместо median
    ...
    def predict(self, plan_dict):
        ...
        # Получаем 1D leaf_id через Grouper
        leaf_id = self.grouper.apply(df_input[strategy_features])[0]

        # 3. Механизм доверия и Fallback
        profile = self.leaf_profiles.get(leaf_id)

        # Если надежность низкая — завышаем память (броня)
        if not profile:
            # or profile['success_rate'] < 0.7:
            pmu_adj = 1.4  # Консервативный буфер 40%
            mt_dop = 2
            threads = 4
            join_mode = 'BROADCAST'
            codegen = 'false'
            conf = profile['success_rate'] if profile else 0
        else:
            ...
            # conf = profile['success_rate']

        # Итоговый расчет памяти (с округлением вверх до 256МБ)
        # final_mem_raw = base_pmu * pmu_adj
        # stable_mem = int(final_mem_raw)
        # int(np.ceil((final_mem_raw * 1.30) / 256.0) * 256)
        if base_pmu < 150:  # Если запрос реально крошечный
            # Используем меньший буфер и меньший шаг
            final_mem_raw = base_pmu * pmu_adj * 1.15
            stable_mem = int(np.ceil(final_mem_raw / 128.0) * 128)  # Шаг 128
        else:
            # Для средних и тяжелых оставляем бронированный вариант
            final_mem_raw = base_pmu * pmu_adj * 1.25
            stable_mem = int(np.ceil(final_mem_raw / 256.0) * 256)

        return {
            'target_mem_limit_dop0': base_pmu,
            'target_mem_limit': f"{stable_mem}mb",
            'target_mt_dop': mt_dop,
            'target_num_scanner_threads': threads,
