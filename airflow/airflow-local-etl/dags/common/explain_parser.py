...
class ExplainParser:
    ...
    def parse(self, explain_rows):
        ...
        features['feature_plan_num_joins'] = full_text.count('JOIN')

        # 2. num_broadcast_joins (Риск OOM)
        features['feature_plan_num_broadcast_joins'] = full_text.count('BROADCAST')

        features['feature_plan_num_scan_nodes'] = full_text.count('SCAN S3') + full_text.count('SCAN HDFS')  # Учитываем оба типа
        features['feature_plan_num_agg_nodes'] = full_text.count('AGGREGATE')

        # 3. num_files (Потенциал параллелизма)
        features['feature_plan_num_files'] = sum([int(x) for x in self.re_files.findall(full_text)])

        # 4. has_missing_stats (Фактор неопределенности)
        features['feature_plan_has_missing_stats'] = 1 if self.re_missing_stats.search(full_text) else 0

        # 5. total_scan_size_bytes (Объем данных)
        sizes = self.re_size.findall(full_text)
        features['feature_plan_total_scan_size_bytes'] = sum([self._convert_to_bytes(s) for s in sizes])

        # 6. max_cardinality (Нагрузка на память / хеш-таблицы)
        cards = self.re_cardinality.findall(full_text)
        features['feature_plan_max_cardinality'] = max([self._convert_cardinality(c) for c in cards]) if cards else 0

        # 7. max_row_size (Вес строки / ширина данных)
        row_sizes = self.re_max_row_size.findall(full_text)
        features['feature_plan_max_row_size'] = max([self._convert_to_bytes(r) for r in row_sizes]) if row_sizes else 0
